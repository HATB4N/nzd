...
void Matrix::setup(
    ...
    */
    ...
    _threads = (_threads>= MAX_T) ? MAX_T : _threads; 
}

void Matrix::forward(
    const std::vector<fp16> &w, const std::vector<fp16> &xt, 
    const std::vector<fp16> &b, std::vector<fp32> &rt) {

        auto _w = std::span<const fp16>(w);
        auto _xt = std::span<const fp16>(xt);
        auto _b = std::span<const fp16>(b);
        ...
        for (unsigned int i = 0; i< _threads; ++i) {
            ...
            futures.emplace_back(std::async(std::launch::async, [=, this](){
                this->product(_w, _xt_part, _b, _rt_part, bs);
            }));
